...
install.gdal<-function(install_path='c:/gdal_map_win',proxy_url=NULL,manual=F){
    if (.Platform$OS.type!='unix'){
        if(R.Version()$arch!='x86_64'){
            ostype<-'win32'
            url.baidupan<-'https://pan.baidu.com/s/1miHGTyK'
        }else{
            ostype<-'win64'
            url.baidupan<-'https://pan.baidu.com/s/1pKR3X6F'
        }
        if(manual){
            cat('Please download and extract zip files into ',install_path,'from your browser..\n')
            browseURL(url.baidupan)
            cat ("Press [enter] to continue if you have already finished the previous step...",'\n')
            line <- readline()
        }else{
            ...
            if(!dir.exists(install_path)){
                ...
                if(!is.null(proxy_url)){
                    ...
                    #clone under proxy
                    repo <- git2r::clone(url = "https://git.oschina.net/seifer_08ms/gisinter_gdal.git",
                                         local_path =install_path,branch = ostype)
